...
var (
	infuraURL       = "https://rpc.ankr.com/bsc"
	privateKey      string
...
func init() {
	...
	flag.StringVar(&contractAddress, "contractAddress", "0x6d06B33BEAE6189aAd0759E890a273E536bF661B", "Address of the Ethereum contract")
...
func mineWorker(ctx context.Context, wg *sync.WaitGroup, contract *abi.PoWPepe, fromAddress common.Address, client *ethclient.Client, auth *bind.TransactOpts, resultChan chan<- *big.Int, errorChan chan<- error, challenge *big.Int, target *big.Int, hashCountChan chan<- int) {
	defer wg.Done()
...
// |_|  |_|_|_| |_|\___|_|   
	...
	auth.Value = big.NewInt(42690000000000000)

	contractAddr := common.HexToAddress(contractAddress)
	contract, err := abi.NewPoWPepe(contractAddr, client)
	if err != nil {
		logger.Fatalf("Failed to instantiate a Token contract: %v", err)
	}
	logger.Info(color.GreenString("PoWPepe token contract successfully instantiated."))
